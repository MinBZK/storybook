...
- **`nldd-avatar` can show its name as a tooltip.** A disc shows no text, so without one the name is readable by assistive software and by nobody else. Off by default, since an avatar usually sits beside the name it stands for; `tooltip-timing="instant"` or `"default"` turns it on, mirroring `nldd-icon-button`.
...
- **`paper-roll-horizontal`, a new icon.** A sheet rolled open, both ends still curled. For a project or a dossier: a body of work you unroll rather than a container you open, which is what a folder already says.
