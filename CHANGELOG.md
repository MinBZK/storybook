...
- **`gap` and `padding` take a step of the spacing scale, in both components.** `nldd-collection` read `gap` as a CSS length and `nldd-container` read it as a scale step, so `gap="16"` was right in one and wrote `--_gap: 16` in the other, which is not a length: the declaration fell away and the space between the items became nothing at all, without a word. One meaning now, behind one resolver. Spacing is rhythm, so a value beside the scale is not a finer choice but a break in the pattern, and a length is refused like any other value that cannot be placed — nothing is written, the default stands, and dev names the value rather than leaving you to spot the collapse. `nldd-collection` gains `sm-gap`, `md-gap` and `lg-gap` on the way, which `nldd-container` already had and which is where a gap that has to vary belongs.
