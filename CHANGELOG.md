...
- **35 new icons** — `accessibility`, `app`, `arrow-left-right`, `binoculars`, `blocks-9`, `book-batch-play`, `brick-wall`, `centralized-network`, `cylinder-2-big-small-split`, `cylinder-split-badge-lock`, `desk-with-screen`, `diamond`, `file-box`, `file-text-batch-check-mark`, `file-text-pencil`, `foundation`, `globe-rack-server`, `hand` (alias `privacy`), `handshake`, `key`, `leaf`, `pencil-ruler`, `pipeline-corner-2`, `pipeline-machine-gear`, `pipeline-valve`, `point-bottom-left-to-point-top-right-s-curve-path` (`s-curve-path`; aliases `path`, `traject`), `radar`, `score-meter`, `seal-check-mark`, `shield`, `shield-lock`, `shopping-cart`, `square-and-arrow-down` (aliases `save`, `import`), `stack-code`, and `table-cells`.
...
- **Icons** — the new icons (listed in Highlights) are normalized to the house format. New aliases for existing icons: `export` (→ square-arrow-up), `settings` (→ gear), and `pipeline-runner` (→ pipeline-machine-gear).
