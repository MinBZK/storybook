...
- **Sixteen icons for the hardware a data center is made of.** `cpu`, `gpu`, `memory-chip`, `psu`, `pci-card`, `transceiver-module`, `rack-server`, `rack-servers`, `storage`, `ssd-hard-drive`, `external-hard-drive`, `network-switch`, `kvm-switch`, `power-plug`, `lightning` and `snowflake`, with the aliases `server`, `servers`, `memory`, `ram`, `processor`, `power`, `cooling`, `airco`, `hard-drive`, `nic`, `network-interface-card`, `sfp` and `energy`. Two are named after what they draw rather than what they came in for: `network-switch`, because a plain `switch` in a design system reads as the toggle first, and `snowflake`. There are two racks: `rack-servers` drawn flat on for a diagram, and `rack-server` at an angle for a row, where three stacked bays turn into three stripes. `server` points at the angled one, `servers` at the flat one.

- **Icons for a stock, a checklist, a board, printing and a firewall.** `boxes-3` for a stock of things you own (`inventory`), `clipboard-bullet-list` for that same list read as a checklist (`inventory-alt`), `kanban-columns` for a board of work in columns (`kanban`), `printer` for printing (`print`), and `shield-arrow-right-arrow-left` for a firewall (`firewall`). That shield carries the traffic it filters, two arrows passing each other, so it says what it does instead of only saying "safe".

- **`network-patch-mapping`, for the cables between two rows of ports.** The runs themselves, sweeping from one side to the other and crossing on the way, rather than the nodes they connect. What you see standing in front of a patch panel.
