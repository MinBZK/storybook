...
- **`nldd-sheet` no longer takes space in the flow.** The sheet is a `position: fixed` dialog, so the host only added an empty box. As a block it was a flex item like any other, and inside an `nldd-split-view-pane` it collected the pane's `::slotted` flex-grow and ate the height its siblings needed. It is `display: contents` now, like `nldd-modal-dialog`.
