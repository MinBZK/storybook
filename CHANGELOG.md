...
- **`nldd-token-field`** — a multi-select input: chosen values render as dismissible `nldd-token`s, options come from a slotted `nldd-menu` (filtered as you type) with a chevron picker button. Keyboard navigation (arrow keys, roving token focus, `Backspace` to step onto and then remove the last token), comma/`Enter` custom values via `allow-custom`, `type` / `autocomplete` / `no-spellcheck`, `readonly` / `required`, `ElementInternals` form participation (one submitted entry per value) and a comma-separated `values` attribute for declarative use.
...
- **`nldd-token`** — a chip that represents a piece of data. Its label comes from a `text` attribute, with the default slot as a fallback. `control="dismiss"` adds a trailing ✕ button (fires `dismiss`, labelled by `dismiss-text`); `control="menu"` adds a trailing chevron button (labelled by `menu-text`) that opens a slotted `nldd-menu` as a popover — the token owns the open/close, `expanded` state and focus return, while the menu items own their own `select`. A label longer than the token truncates with an ellipsis, and the host shows a focus ring when focused, forced on a scripted focus so roving-focus containers such as `nldd-token-field` highlight the whole token (incl. in Safari).
