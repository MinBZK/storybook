...
- **`modeless` is gone from `nldd-sheet`.** A sheet is always modal now: it opens with `showModal()`, draws a backdrop and holds focus. The attribute promised a sheet you could leave open while working behind it, and the native `<dialog>` gives you the shell for that but none of the things that make it usable. Nothing in this system used it, and the one story that showed it warned in its own text that it did not lay out correctly. Whatever it was reaching for wants solving on its own terms rather than as a flag on a component built to be modal. `nldd-window` keeps its `modeless`, which is the component for a panel you work beside.
