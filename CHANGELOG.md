...
- **Fifteen icons for the hardware a data center is made of.** `cpu`, `gpu`, `memory-chip`, `psu`, `pci-card`, `transceiver-module`, `rack-servers`, `storage`, `ssd-hard-drive`, `external-hard-drive`, `network-switch`, `kvm-switch`, `power-plug`, `lightning` and `snowflake`, with the aliases `server`, `servers`, `memory`, `ram`, `processor`, `power`, `cooling`, `airco`, `hard-drive`, `nic`, `network-interface-card`, `sfp` and `energy`. A catalog of parts had to reach for a box, a folder or a plug for all of them, which made a list of components look like a list of files. Two are named after what they draw rather than what they came in for: `network-switch`, because a plain `switch` in a design system reads as the toggle first, and `snowflake`.

- **`rack-server`, a rack in perspective.** The set had `rack-servers`, drawn flat on with three bays stacked, which is right for a diagram and busy in a list row, where three bays turn into three stripes. This one stands at an angle, with the front panel and its status light at the bottom. `server` moves to it, `servers` stays on `rack-servers`.

- **Five icons for parts of an app that had no picture yet.** `boxes-3` for a stock of things you own (`inventory`), `clipboard-bullet-list` for that same list read as a checklist (`inventory-alt`), `kanban-columns` for a board of work in columns (`kanban`), `printer` for printing (`print`), and `shield-arrow-right-arrow-left` for a firewall (`firewall`). That shield carries the traffic it filters, two arrows passing each other, so it says what it does instead of only saying "safe".

- **`network-patch-mapping`, for the cables between two rows of ports.** The choice used to be `list`, which every list reaches for, or `network-structure`, which draws nodes instead of the runs themselves. This is what you see standing in front of a patch panel.

- **The three task states as one series.** `circle` (not started), `circle-circle` (under way) and `check-mark-circle` (finished), with `minus-circle` as the barred variant. Aliased as `to-do` and `doing`. Prefer `circle-circle` over `minus-circle` for work in progress, because a ring with a bar through it means "no entry" everywhere else. `circle` doubles as `no-priority`: the same ring as `low-priority` through `high-priority` with nothing in it, so that column keeps reading as a count from zero up.
...
- **`namespace` as an alias for `brackets-ellipsis`, `all-tasks` and `done` as aliases.** The brackets with the ellipsis stand for a Kubernetes namespace wherever the console uses them, and the only name on offer was `embed`. `circle-grid-2x2-top-left-check-mark` and `check-mark-circle` get the names a task menu is written in.

- **`disabled` on `nldd-list-item`.** A segment could be switched off and a row could not, while a row can just as well be the button or the checkbox. A `button` or `checkbox` row takes the real off state and dims. An `href` row gets `aria-disabled` and a blocked click, because an anchor cannot be switched off the way a button can. Hover and press do nothing there, and the arrow keys skip it: a disabled control is not focusable, so a stop there would swallow the key. That already applied to a disabled segment, so a row whose only segment is off is no longer a stop either.
