...
- **Een badge in een kleur die het systeem niet kent.** Er zijn kleuren die geen categorie zijn maar een feit: de mantel van een kabel is blauw, en dat blijft zo hoe de huisstijl ook verschuift. `custom-color` neemt elke CSS-kleurwaarde aan (`#3b82f6`, `oklch(...)`, een eigen var) en wint van `color`, zodat de woordenlijst van `color` dicht blijft en je in één zoekopdracht ziet waar een app buiten het palet stapt. Wat erop staat wordt wit of zwart, wat het beste contrasteert: de badge zet de kleur op zichzelf en laat `--semantics-content-contrast-color` het rekenwerk doen, dezelfde formule die `nldd-avatar` al gebruikt. Daarnaast kent `color` nu ook `inherit`, met de betekenis die het bij de avatar heeft: vul met de contentkleur eromheen (`--context-content-color`, anders `currentColor`), zodat een badge meereist met de regel waarin hij staat.

- **`rack-server`, a rack drawn in perspective.** The set had one picture of a rack, `rack-servers`, drawn flat on: three bays stacked with a light in each. That is the right drawing for a diagram and a busy one in a list row, where three bays turn into three stripes. This is the same subject at an angle, with the front panel and its status light at the bottom and the body receding above it. `server` moves to it, `servers` stays on `rack-servers`.
- **Five icons for the parts of an app that had no picture yet.** `boxes-3` for a stock of things you own (`inventory`), `clipboard-bullet-list` for the same list read as a checklist (`inventory-alt`), `kanban-columns` for a board of work in columns (`kanban`), `printer` for printing (`print`), and `shield-arrow-right-arrow-left` for a firewall (`firewall`). Each was being drawn with the nearest thing at hand: a folder for a stock, a table for a board, a plain shield for a firewall, which says "safe" without saying what does the work. The firewall shield carries the traffic it filters, two arrows passing each other, so it reads as a thing that lets some through and stops the rest.
- **The three shields are redrawn on one silhouette.** `shield`, `shield-check-mark` and `shield-lock` were a pointed crest drawn from arcs; they now share the flat-topped, rolled-edge shield that `shield-arrow-right-arrow-left` uses. Four shields that differ only in what they carry, rather than one family that splits in two the moment a fourth is added. `psu` was redrawn along with them: the socket sits lower in the housing, so the plug reads as seated rather than floating.
- **`network-patch-mapping`, for the cables that run between two rows of ports.** A patch section had a choice between `list`, which is what every other list in an app reaches for, and `network-structure`, which draws nodes and the lines between them rather than the runs themselves. This is what you see when you stand in front of a patch panel: cable runs sweeping from one side to the other and crossing on the way.
- **Fifteen icons for the hardware a data center is made of.** `cpu`, `gpu`, `memory-chip`, `psu`, `pci-card`, `transceiver-module`, `rack-servers`, `storage`, `ssd-hard-drive`, `external-hard-drive`, `network-switch`, `kvm-switch`, `power-plug`, `lightning` and `snowflake`. A catalog of parts had to reach for a box, a folder or a plug for all of them, which made a list of components look like a list of files. Aliases carry them outside a catalog too: `server`, `servers`, `memory`, `ram`, `processor`, `power`, `cooling`, `airco`, `hard-drive`, `nic`, `network-interface-card`, `sfp` and `energy`. Two are named after what they draw rather than what they were called on arrival: `network-switch`, because a plain `switch` in a design system is read as the toggle first, and `snowflake`.
- **`nldd-text`, for the body text that had no component.** Everywhere an app writes a line that is not a heading and not a block of prose — under a title in a list row, in a panel, as a caption — it wrote a bare `<p>` and inherited whatever the page happened to set. This names it: `size` (xxs through lg), `weight` (regular, medium, bold), `line-height` (flat, tight, snug, loose), `color` and `horizontal-alignment`. Every combination it offers already exists as a token, so it cannot invent typography, only name what the scale has. Color follows the content channel a list item, menu or table sets on its content, so a line inside a row travels with that row as it lights up, and `color="inherit"` is there for text on a painted surface. A slotted `<strong>` or `<b>` takes the bold weight from the scale rather than the browser's relative "bolder". For a heading reach for `nldd-title`, for a block of prose with its own rhythm `nldd-rich-text`.
- **A plain `circle`, for the priority that is not set.** The set already draws priority as a count: `low-priority` is one exclamation mark in a circle, `medium-priority` two, `high-priority` three. What it could not draw was zero. A task nobody prioritized had to borrow a shape from somewhere else, which is how a dashed circle or a warning triangle ends up in a column that is otherwise about how many marks there are. This is the same ring as the rest of the family with nothing in it, so the column keeps reading as a count from zero up. Aliased as `no-priority`.
- **A light weight for the five state icons, drawn for 32 pixels.** `circle-light`, `circle-circle-light`, `check-mark-circle-light`, `clock-light` and `slash-circle-light` keep the 2-pixel line of the regular set on a 32 view-box instead of a 24 one, so the line stays 2 pixels at the size they are drawn for rather than growing to 2.7 with the icon. Use them where a state icon leads a list row at 32; below that reach for the regular set, which is what a 24 view-box is for. Aliased as `to-do-light`, `doing-light`, `done-light` and `blocked-light`, matching `to-do`, `doing`, `done` and `blocked`.
- **`circle-circle` and `minus-circle`, the middle of three states.** A task is not started, under way, or finished, and the set could draw the first and the last: `circle` and `check-mark-circle`. In between it had nothing, so a menu of the three had to switch shape halfway and stopped reading as one series. Both fill that gap with the same ring: `circle-circle` puts a second ring inside it, `minus-circle` a bar across it. Prefer `circle-circle` for work under way, aliased as `doing`. A ring with a bar through it is a road sign everywhere else, and "no entry" is not what a task in progress means. `to-do` is added as an alias of `circle`, so the three states can be named as one set where they are used.

### Breaking

- **`nldd-box` paints with `background`, like every other surface.** The attribute was `variant`, which in this system is the word for what a component *is* (a button is primary or destructive), not for which surface it draws. `nldd-app-view`, `nldd-page`, `nldd-split-view-pane` and `nldd-card` all call that `background`, and the box sets exactly the same thing: a surface colour and a border colour from one palette, for all three of its values. So `variant="base|tinted|critical"` is now `background="base|tinted|critical"`; `critical` stays a value there, the way it is a colour on a badge, a tag and a text. One thing to know when reading the two side by side: a box starts on `tinted` where a card starts on `base`, because a box is there to stand out from the page and a card to sit on it. Consumers change one string per box that sets it.
...
- **A segmented action stays lit while what it opened is on screen.** `expanded` on `nldd-list-item-action` set `aria-expanded` and turned a chevron, but left the segment looking untouched, so a menu hanging off one row floated over the list without saying which row it belonged to. It now paints too, one step above hover (`--components-list-item-is-expanded-background-color`, between the hovered and the selected fill) and after the hover rule, so moving the pointer back over the segment cannot dim it while the menu is still open.
...
- **`readonly` on `nldd-combo-box`.** A value that belongs to the record rather than to the form (the product an asset is an instance of, say) could only be locked with `disabled`, which greys the field out, drops it from the tab order and hands a screen reader nothing to read. The combo box now takes `readonly` like `nldd-text-field`, `nldd-date-field` and `nldd-time-field` do: the value stays readable, selectable and reachable with the keyboard, and the read-only surface says why it does not move. The menu does not open, however you ask for it, there is nothing to clear, and leaving the field leaves the text alone: a read-only field takes no keys and reverts nothing, so a label that matches no menu option survives a click elsewhere.
- **A read-only field takes its controls away instead of dimming them.** The calendar button of `nldd-date-field`, the picker of `nldd-time-field` and the chevron of `nldd-combo-box` were rendered disabled when the field was read-only: a button that can never do anything, taking up the space where the value could have run on. They are gone now, which is what `nldd-token-field` already did with the dismiss control on a read-only token. Disabled keeps its buttons, because those come back: read-only says the value is not yours to change here, disabled says this control is not available right now.
- **`keyboard` and `enter-key` on the text fields.** A rack unit, a house number and a postcode are digits without being a quantity you would step, so they belong in a text field. On a phone that field opened the letter keyboard, and the only way around it was `type="tel"`, which asks for a phone number and formats like one. `keyboard` forwards `inputmode` (`none`, `text`, `decimal`, `numeric`, `tel`, `search`, `email`, `url`) and `enter-key` forwards `enterkeyhint` (`enter`, `done`, `go`, `next`, `previous`, `search`, `send`), on `nldd-text-field` and `nldd-multi-line-text-field`. They are hints to the virtual keyboard and nothing else: what the field accepts does not change, and on a desktop neither does anything. Named after what they do rather than after the HTML attribute, the way `accessible-label` is. `type` already asks for the right keyboard for an email address, a phone number and a URL, so setting both to different things now warns in dev.
...
- **`nldd-icon`** — `size="full"` names the existing behaviour (fill whatever sizes you) and `size="inherit"` makes the icon follow the surrounding text (1em), for an icon set in a sentence. Reach for the latter rather than your own `nldd-icon { width: 1em }` rule: such a rule wins over the component and also shrinks the icons a cell or button was already sizing correctly.
