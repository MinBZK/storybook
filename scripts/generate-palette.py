...
- Step-based "plateau" chroma curve: peak (= reference chroma) at step 500,
  flat plateau out to the reference color's natural step (and its mirror
  around 500), then linear taper to the extremes
- Special "neutral" mode with L-based asymmetric chroma falloff for grays
...
def position_for_luminance(target_lum: float, center_lum: float) -> float:
	"""Inverse of the log-luminance curve: which palette position (0..1) hits this lum?"""
	log_white = math.log(1.0 + 0.05)
	log_center = math.log(center_lum + 0.05)
	log_black = math.log(0.0 + 0.05)
	log_target = math.log(max(target_lum, 0.0001) + 0.05)

	if log_target >= log_center:
		t = (log_target - log_white) / (log_center - log_white)
		return max(0.0, min(0.5, t * 0.5))
	t = (log_target - log_center) / (log_black - log_center)
	return max(0.5, min(1.0, 0.5 + t * 0.5))


def generate_palette(hex_color: str, is_neutral: bool = False):
	"""Generate a 25-color palette from a reference color.

	Two chroma strategies:

	- Non-neutral: a step-based "plateau" curve. The reference color's natural
	  position (the step where its luminance lands) is mirrored around step 500.
	  Between mirror and reference position chroma_factor = 1.0 (so step 500 is
	  always at peak chroma, equal to C_ref); outside that range it tapers
	  linearly to ~0.05 at the extremes. This makes step 500 the most vibrant
	  step while preserving the brand color's chroma at its natural step.

	- Neutral: an asymmetric L-based falloff (more aggressive on the dark side)
	  for grayscale palettes. Unchanged.
	"""
	rgb = hex_to_rgb(hex_color)
	L_ref, C_ref, H_ref = rgb_to_oklch(rgb)
	ref_lum = relative_luminance(rgb)

	center_lum = math.sqrt(1.05 * 0.05) - 0.05

	ref_position = position_for_luminance(ref_lum, center_lum)
	plateau_radius = abs(ref_position - 0.5) / 0.5
	...
	for i, label in enumerate(labels):
		if i == 0:
			...
			if not is_neutral:
				distance_from_500 = abs(position - 0.5) / 0.5
				if distance_from_500 <= plateau_radius:
					step_chroma_factor = 1.0
				else:
					taper_t = (distance_from_500 - plateau_radius) / (1.0 - plateau_radius)
					step_chroma_factor = 1.0 - taper_t * 0.95
			...
			for _ in range(50):
				...
				if is_neutral:
					...
					chroma_factor = step_chroma_factor
