...
- **Three new icons.** `ship-wheel-badge-plus` and `puzzle-piece-badge-plus` are the ship's wheel and the puzzle piece with a plus in the corner: adding a cluster, installing a plugin. The badge sits where a badge sits on every other icon in the set, so the thing itself stays recognisable and the plus only says what you are about to do with it. `seal-star` is that same seal as `seal-check-mark` with a star in it: where the check mark says something was verified, the star says something is the organisation's own. Also reachable as `add-kubernetes` / `add-k8s`, `add-plugin` and `brand`.
