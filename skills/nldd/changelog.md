...
- **Seventeen icons for a data center: the hardware, and the cables between the ports.** `cpu`, `gpu`, `memory-chip`, `psu`, `pci-card`, `transceiver-module`, `rack-server`, `rack-servers`, `storage`, `ssd-hard-drive`, `external-hard-drive`, `network-switch`, `kvm-switch`, `power-plug`, `lightning`, `snowflake` and `network-patch-mapping`, with the aliases `server`, `servers`, `memory`, `ram`, `processor`, `power`, `cooling`, `airco`, `hard-drive`, `nic`, `network-interface-card`, `sfp` and `energy`. Two are named after what they draw rather than what they came in for: `network-switch`, because a plain `switch` in a design system reads as the toggle first, and `snowflake`. There are two racks: `rack-servers` drawn flat on for a diagram, and `rack-server` at an angle for a row, where three stacked bays turn into three stripes. `server` points at the angled one, `servers` at the flat one. `network-patch-mapping` draws the runs themselves, sweeping from one side to the other and crossing on the way, rather than the nodes they connect.
...
- **The three task states as one series.** `circle` for to do, `circle-circle` for doing and `check-mark-circle` for done, with `minus-circle` as the barred variant. Aliased as `to-do` and `doing`, matching `done`. Prefer `circle-circle` over `minus-circle` for the doing state, because a ring with a bar through it means "no entry" everywhere else. `circle` doubles as `no-priority`: the same ring as `low-priority` through `high-priority` with nothing in it, so that column keeps reading as a count from zero up.
