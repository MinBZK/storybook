...
- **`nldd-time-field` and `nldd-time-picker`** — a text field for a time, with an optional picker in a popover. The time-twin of `nldd-date-field` and `nldd-date-picker`: the value is 24-hour `HH:mm`, typing is read generously (`9`, `930`, `9u30`, `9.30`) and normalised when you leave the field, and `min` / `max` / form participation work the same way. `step` sets the minute step; typed input rounds to it on commit, the arrow keys move by it, and the picker only offers minutes that land on it. The step counts from `min`, so a series that starts at :07 works too, the way `<input type="time">` does it. At the end of the day rounding goes down rather than wrapping to 00:00, which would silently shift the day next to a date field. The picker is a wheel: two columns of values sliding past a band in the middle, where scrolling is choosing. CSS scroll snapping does the work; the component only reads which value ended up in the middle. The band carries the semantics, because that is what a wheel is: hour and minute are each a spinbutton, and the columns underneath are `aria-hidden` so the same values are not announced twice. `no-picker` drops the button on the field, and a `picker` slot takes your own.

  Nothing inside the wheel closes the popover. Scrolling fires `input` and settling on a value fires `change`, but a time has two halves, so setting the hour is half an answer: the "Klaar" button under the wheel is what ends it. That is the difference with the calendar, where one click on a day is the whole answer. Up and down set the value, left and right move between hour and minute, the same split as the segments of a native `<input type="time">`. Rows are 44px on a touch pointer and 32px on a fine one, the same split `nldd-menu` makes, so a finger gets the full target and a mouse gets more values in the same height. `rows` sets how tall the columns are, in rows (7 by default, 3 at the least). The chosen value always sits in the middle, so an odd count shows whole rows and an even one clips half a row off the top and bottom.
