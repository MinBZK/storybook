...
- **A run of fixes with one cause.** Enter in a single-line field submits the form again, a click on a field's label reaches every input, a field's label arrives at the control as its accessible name, a toolbar reflows when the text on a control gets shorter, and a menu item no longer claims a menu role outside a menu. Different components, one habit: each of them worked out what something was instead of asking it. `nldd-form-field` recognized an input by an implementation detail that 8 of the 24 happened to have, so for the rest the label reached nobody and a screen reader announced an unnamed field. `nldd-toolbar` guessed which attribute meant "wider" and missed the one that did. `nldd-menu-item` assumed a menu stood around it. They now ask, or measure: an input says `static isFormInput = true`, the toolbar watches the width its children actually take, and a menu item waits to be claimed by a menu. Reported by robbertbos, uittenbroekrobbert and rubenrouwhof, each of them from their own application.
