...
- **New icons.** `bug`, `square`, `square-1` (alias `primary`), `triangle-square-circle` (alias `categories`), `book-badge-plus` (alias `new-book`) and `tag-stack` (aliases `tags` and `labels`, for a set of labels where `tag` is one), plus the aliases `add-text-document` for `file-text-badge-check-plus` and `announcement` for `megaphone`. The alias `category` for `tag` is gone: a label is not a category, and beside the new `categories` (`triangle-square-circle`) the two read as the same thing. Use `tag` or `label`.
