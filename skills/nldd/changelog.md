...
- **At the end of a URL in `nldd-text-editor` the caret sits against the URL when you are editing it.** The open-link badge stands at the link's end, and the caret there had two places to be: against the URL or after the badge. It was always drawn after the badge, whatever you were doing, so deleting the last character of a URL looked like editing outside the link while the next Backspace ate the URL again. The badge now answers for both sides, so arriving from the left or deleting backward puts the caret against the URL, and arriving from the right puts it past the badge, where typing continues after the link. (#214)
