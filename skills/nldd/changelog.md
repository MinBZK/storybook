...
- **`gap` and `padding` read a scale step or a length, and say so when they can read neither.** `nldd-collection` took a CSS length, `nldd-container` took a step of the spacing scale, and both called it `gap`: `gap="16"` was right in one and wrote `--_gap: 16` in the other, which is not a length, so the declaration fell away and the space became nothing at all without a word. One rule now, in one resolver behind both, and it reads off the value itself: a bare number is a step, anything carrying a unit is a length of your own. A bare number that is no step is refused rather than written, because `var(--primitives-space-23)` is that same silent zero in a new coat, and dev says which value it could not place. `nldd-collection` gains `sm-gap`, `md-gap` and `lg-gap` along the way, which `nldd-container` already had.
