...
- **Sixteen icons for the hardware a data center is made of.** `cpu`, `gpu`, `memory-chip`, `psu`, `pci-card`, `transceiver-module`, `rack-server`, `rack-servers`, `storage`, `ssd-hard-drive`, `external-hard-drive`, `network-switch`, `kvm-switch`, `power-plug`, `lightning` and `snowflake`, with the aliases `server`, `servers`, `memory`, `ram`, `processor`, `power`, `cooling`, `airco`, `hard-drive`, `nic`, `network-interface-card`, `sfp` and `energy`. A catalog of parts had to reach for a box, a folder or a plug for all of them, which made a list of components look like a list of files. Two are named after what they draw rather than what they came in for: `network-switch`, because a plain `switch` in a design system reads as the toggle first, and `snowflake`. There are two racks: `rack-servers` drawn flat on for a diagram, and `rack-server` at an angle for a row, where three stacked bays turn into three stripes. `server` points at the angled one, `servers` at the flat one.
