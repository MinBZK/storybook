...
- **`avatar`, `icon` and `symbol` on a typeahead candidate.** A person or organization shows as an `nldd-avatar` (an image, or initials from the label; `avatar: {}` is enough), and its row takes two lines, the `detail` under the label as supporting text. A channel shows as an `nldd-icon` at the size of a menu item's, an emoji as itself (`symbol`), in front of the label, so a list is scannable at a glance. The built-in mention takes them too. (#204)
