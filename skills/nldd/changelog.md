...
- **New icons.** `bug`, `square`, `square-1` (alias `primary`), `triangle-square-circle` (alias `categories`) and `book-badge-plus` (alias `new-book`), plus the aliases `add-text-document` for `file-text-badge-check-plus` and `announcement` for `megaphone`.
