...
- **`nldd-form-field-error-text` is on its way out.** An error you have to fix is a requirement the value does not meet, so it belongs in an `nldd-validation-list` alongside the requirements that can state themselves. Both still work; the error text will go in a later release. Rules that a client can check, a length or a format, become an item that checks itself, and anything only a server can decide becomes an item without a rule that you name in `unmet`.
