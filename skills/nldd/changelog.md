...
- **`nldd-validation-list`, everything a value has to satisfy in one list.** A requirement you can state up front is an item with a rule, `minlength`, `maxlength`, `match` or `required`, and it checks itself while you type. One only a server can decide is an item without a rule, and the app names it in `unmet` on the control. The list has two modes and `judging` is the switch: before a verdict it states what the field wants, after one those hints are gone for good and what is left is whatever the value fails. What turns red is `invalid` on the control, and only that, so a line never goes critical under a field that looks fine. Hints are off by default, because a phone number does not need its format spelled out before anyone has typed and a password does. The list throws that switch itself once its control is `invalid`, so when a field starts being judged is decided by whoever writes that attribute, and the moment for it is on submit rather than while someone is still typing. There are no checkmarks: the control already shows a validation icon, and repeating it per line says the same thing three times.
...
- **`error-message` on an input is now `unmet`, and there is no fallback.** It names the ids of the items a value does not satisfy, which is what an `nldd-validation-list` reads. The old name sat one letter away from `error-message-ids`, an internal property that does the opposite: that one is written by `nldd-form-field` and read by nobody. Nothing warns when the old attribute is left in place. `nldd-form-field` simply reads an attribute that is not there, so every error message on the page silently stops appearing, with no console output and no build error. Consumers replace the attribute wherever they set it: `error-message=` becomes `unmet=`, and in a framework template the bound form too, so Angular's `[attr.error-message]` becomes `[attr.unmet]`. In a repository that is one search and replace on `error-message`, minding that `error-message-ids` is a different thing and keeps its name.

- **`nldd-form-field-error-text` is gone.** An error you have to fix is a requirement the value does not meet, so it belongs in an `nldd-validation-list` alongside the requirements that can state themselves. Two ways to show one message is exactly what a consumer trips over, and being of one mind about it is the whole point of a design system. Migration is per field, and the ids travel with the texts: a rule a client can check becomes an item that checks itself, so `<nldd-form-field-error-text id="password-length">Must be at least 8 characters.</nldd-form-field-error-text>` becomes `<nldd-validation-item id="password-length" minlength="8">At least 8 characters</nldd-validation-item>`, and anything only a server can decide becomes an item without a rule that stays named in `unmet` on the input. Nothing warns, and leaving the old element in place is worse than a blank: an undefined custom element has no shadow styles, so the `display: none` that used to hide it is gone and every error message on the page renders permanently, whether or not the field is invalid. Search for the tag rather than waiting for a console message.

- **Validation texts are the requirement, not the instruction.** The imperative belongs on buttons, where it announces something you set in motion. An item states what the value has to be, because the same line does two jobs: a requirement up front and, once the field is judged, what is still wrong. So "Use at least 8 characters" becomes "At least 8 characters", "Pick at least one option" becomes "At least one option", and "Enter a valid company number (8 digits)" becomes "A company number of 8 digits". This touches every message a consumer has already written; the examples are in `skills/nldd/SKILL.md`.
...
- **The track lines in `nldd-timeline-track-cell` get the same ring, on their sides only.** The dot already had one so it reads over the line running under it; the line itself had nothing, so it ran straight into whatever sat beside it. A spread would have put the same band above and below, and there it would cut the track at every row: the lines that run on downward deliberately reach past the cell to bridge the divider. Two offset copies of the line paint the sides and nothing else, because a copy shifted sideways has the same top and bottom as the line and can only show along the edge it is pushed out from.

- **`--semantics-surfaces-ring-thickness`, one token for the gap around something that overlaps.** A badge over its anchor, an avatar over its neighbour, a timeline dot over the line running under it, a step marker over its track: four components draw the same spread box-shadow in the surrounding background so the shape in front stays readable. They took the value from two different primitives, a spacing step and a border width, and called it two different things, `--_ring-thickness` and `--_ring-width`. Both were 2px, so nothing looked wrong; they simply disagreed about what a ring is. The color stays in the components on purpose: it has to be the background actually behind the element, which they read from `--context-parent-background-color`, and a `var()` at the token level would freeze against `:root` and always land on the fallback.

- **`width="fit-content"` on `nldd-date-field` and `nldd-time-field`.** Both hold room at their end for a validation icon, so the field does not resize the moment it turns valid or invalid. Where a field is never validated that room is a gap you can see, between the date and the picker button. `fit-content` keeps only the air the text needs there, and takes the full slot back the moment a validation state does arrive — so the field grows, at its trailing edge, leaving the date and a range's separator and end date where they were. Opt-in, because only the consumer knows whether a field is ever validated; the default still holds the room. Not the CSS keyword but this system's width word, the same one `nldd-cell` and `nldd-segmented-control` already speak: as wide as what is in it.
...
- **`required`, `pattern`, `minlength` and `maxlength` now stop a submit.** They are handed to a control inside the shadow root, and that control is not a member of the form around the component: the form sees the host, the host sees the control, and nothing joined the two. So a `required` field submitted empty, the browser never fired `invalid` on it, `nldd-form` never marked it, and nothing on screen said why. The form-associated mixin now reports the control's verdict as the host's own, which fixes it for every input that renders one: the text fields, the password, search, combo box and multi-line fields, the checkbox, switch and radio button, and the date and time fields. A group (`nldd-segmented-control`, `nldd-toggle-button-group`) and the editors have no such control and are unchanged. Two consequences worth knowing: a form that used to go through with an empty required field now stops, which is the point; and outside an `nldd-form` the browser shows its own bubble, the way it does for any form control. Inside one it does not, because `nldd-form` already cancels that event and writes the message under the field instead.

- **A validation list and a native constraint no longer erase each other.** `setValidity` writes the whole set of flags at once, so whoever wrote last won: a list reporting its failing rule cleared the field's `required`, and the next render cleared the rule. Both now go through `setCustomValidity`, the platform's own name for a reason of your own, which the mixin keeps beside the native flags rather than in place of them. A plain native input answers to the same method, so a list outside this system's inputs still works.
- **A validation list keeps listening after its field is moved.** Taking the element out of the DOM and putting it back, which is what a consumer portalling a sheet to `document.body` does, runs `disconnectedCallback` and takes the input listener with it. Reconnecting looked at the control, found the same one as before, concluded there was nothing to wire up, and left the list reacting to nothing: rules stopped updating as you typed and `invalid` on the control no longer reached it. It now also checks whether it still has an observer.

- **The website title in `nldd-top-navigation-bar` is a 24 by 24 target.** WCAG 2.5.8 asks that of anything you click, and this one was 20 high on a phone and 23 at medium. The row around it was 24, but that came from padding on the bar, which does nothing for the size of the target inside it. The title now carries its own minimum in both directions, on every screen and whether or not it links anywhere: a title that changed height depending on its href would move the whole bar with it. On a small screen the four pixels sit on top, so the text keeps its place and the box grows downward; padding on both sides would have made it 28, since with border-box a 20-pixel line and two paddings of 4 no longer fit in 24. Without a logo above it the bar keeps 6 pixels of its own, which is what the focus ring needs to stand clear.
- **Avatars in a group overlap by a fifth of their diameter, whatever that diameter is.** The overlap was a flat 8 pixels, which meant it was 29% of the avatar at size 28 and 8% at size 96: the same component read as a tight stack at one size and a row of separate circles at another. There was a step meant to make the small sizes overlap less, and it did the opposite, giving 28 more overlap than 24. A fifth is what 8 was at the default size of 40, so a group at that size looks exactly as it did and every other size now follows it.
- **A form section stands 24 pixels from its neighbours on both sides.** It carried padding on both sides, and padding collapses with nothing, so it added itself to the 16 the form already puts under the preceding child. It is a margin now, which does collapse: 16 and 24 beside each other are 24. With the dividers gone there was nothing left for the padding to hold apart anyway. On both sides rather than only above, because with a top margin alone the distance under a section depended on what happened to follow it: another section or the buttons bring their own 24, but a plain field says nothing and the general 16 stayed.
...
- **A form section spaces whatever you put in it.** Its children leaned on a rule that only recognized one field followed by another, so a button or a paragraph inside a section got no room and every caller had to add it themselves. The section now uses the same rhythm as the form around it, which is what it is: a group inside that same form. It asks whether something visible follows, not what the thing is.
...
- **One gap between everything in a form.** Two fields in a row used to sit ten pixels apart instead of sixteen: on every screen under 641px, and above it whenever both carried their label on top. It took a rule with two container queries and a second one with four `:not()`s to say that, and it bought six pixels per pair. Five fields under each other saved thirty pixels, less than a third of one field, and in exchange a toggle group or a combo box, which are far heavier blocks than a text field, stood ten pixels from the next label. A field, a section, a button, a paragraph: whatever follows another thing now stands the same distance from it, and a section or the buttons of a form stand a step further off. The three gap tokens are two, named after what they separate rather than after how big they are next to each other: `--semantics-forms-fields-gap` and `--semantics-forms-sections-gap`. `--semantics-forms-gap` and `--semantics-forms-gap-tight` are gone. Nothing outside this package used either of them.
- **`nldd-form-section` no longer draws lines around itself.** A section carries a heading, and a rule above and below it says the same thing a second time. They also only appeared in forms that use sections, so one application ended up with two kinds of form: most with nothing but whitespace between the fields, a couple with lines through them, and the difference said nothing about the content. What gave it away was the suppressing: no line on the first child, none between two adjacent sections, none on the last. Three exceptions to hide a line everywhere it looked wrong, and the last one did not work. A section is almost never the last child, because an `nldd-form-actions` follows it, so every form with a submit button had a rule drawn right above that button. The padding stays and is what groups: a section gets more air around it than two fields get between them.
...
- **A bar in a sheet keeps its back button.** A split view hides the back button of a bar inside a pane, because the menu beside you is the way back. That hiding travels down the whole pane, so a sheet standing in one lost the back button of its own bar, where there is no menu beside you. A sheet starts that context over.
...
- **Lit no longer reports an update scheduled after an update completed.** `nldd-list`, `nldd-list-item`, `nldd-collection`, `nldd-menu`, `nldd-menu-item` and `nldd-navigation-split-view` set state from `firstUpdated`, from the first `slotchange` or from `updated`, which asks for a second render on top of the one that just finished. That is a dev-mode warning on every page holding a list or a menu. What the first render needs is now read from the light DOM before it, and what can only be read afterwards, such as a slot or a measured width, is set out of the update cycle.
...
- **A `close` from an overlay stays with the overlay it came from.** `nldd-sheet`, `nldd-window`, `nldd-modal-dialog` and `nldd-popover` fired `close` with `bubbles: true`, so an overlay inside another one reached the listener on the one around it too. Overlays nest all the time: a floor plan in a sheet with a form for one room over it, a date picker inside a dialog. The component itself was never fooled, since it checks whether a close came from its own dialog, but a consumer binding `(close)` on the outer overlay got the inner one's close as well and tore down what was still open. The event no longer bubbles: a listener on an overlay hears that overlay and nothing under it. Anyone who was checking `event.target` to work around this can stop; the check is now harmless rather than necessary.
