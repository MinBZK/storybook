...
- **`nldd-token` label truncation and focus ring** — a label longer than the token truncates with an ellipsis, and the host shows a focus ring when focused (for roving-focus containers such as `nldd-token-field`).
