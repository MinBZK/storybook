...
- **`nldd-container` masonry** — `layout="masonry"`: native CSS grid-lanes masonry where supported, CSS multicol fallback otherwise (CSS-only, no JS). Honours `gap` on both axes and `column-count`.
