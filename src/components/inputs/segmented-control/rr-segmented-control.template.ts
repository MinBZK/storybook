...
export function segmentedControlItemTemplate(component: RRSegmentedControlItem): TemplateResult {
	const isIcon = component.contentVariant === 'icon';
	...
	return html`
		...
		>
			...
			<span class="segmented-control__item-icon"
				aria-hidden=${component.contentVariant === 'icon' ? nothing : 'true'}
			>
				<slot name="icon"></slot>
			</span>
			<span class="segmented-control__item-text"
				aria-hidden=${component.contentVariant === 'text' ? nothing : 'true'}
			>
