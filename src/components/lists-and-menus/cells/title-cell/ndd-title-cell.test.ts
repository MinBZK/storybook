...
describe('ndd-title-cell', () => {
	...
	it('reflects size attribute', async () => {
		el = await fixture('<ndd-title-cell size="2"></ndd-title-cell>');
		await waitForUpdate(el);
	...
	it('reflects color attribute', async () => {
		el = await fixture('<ndd-title-cell color="inherit"></ndd-title-cell>');
		await waitForUpdate(el);
	...
	it('reflects horizontal-alignment attribute', async () => {
		el = await fixture('<ndd-title-cell horizontal-alignment="right"></ndd-title-cell>');
		await waitForUpdate(el);
	...
	it('reflects vertical-alignment top', async () => {
		el = await fixture('<ndd-title-cell vertical-alignment="top"></ndd-title-cell>');
		await waitForUpdate(el);
		expect(el.getAttribute('vertical-alignment')).toBe('top');
	});

	it('reflects vertical-alignment bottom', async () => {
		el = await fixture('<ndd-title-cell vertical-alignment="bottom"></ndd-title-cell>');
		await waitForUpdate(el);
	...
	it('renders overline in shadow DOM', async () => {
		el = await fixture('<ndd-title-cell overline="Overline"></ndd-title-cell>');
		await waitForUpdate(el);
	...
	it('renders supporting-text in shadow DOM', async () => {
		el = await fixture('<ndd-title-cell supporting-text="Supporting"></ndd-title-cell>');
		await waitForUpdate(el);
	...
	it('renders a <p> by default (no heading-level)', async () => {
		el = await fixture('<ndd-title-cell text="Title"></ndd-title-cell>');
		await waitForUpdate(el);
		const title = el.shadowRoot!.querySelector('.title-cell__title');
		expect(title).not.toBeNull();
	...
	it('renders an <h1> when heading-level="1"', async () => {
		el = await fixture('<ndd-title-cell text="Title" heading-level="1"></ndd-title-cell>');
		await waitForUpdate(el);
		const title = el.shadowRoot!.querySelector('.title-cell__title');
	...
	it('renders an <h2> when heading-level="2"', async () => {
		el = await fixture('<ndd-title-cell text="Title" heading-level="2"></ndd-title-cell>');
		await waitForUpdate(el);
		const title = el.shadowRoot!.querySelector('.title-cell__title');
	...
	it('renders an <h6> when heading-level="6"', async () => {
		el = await fixture('<ndd-title-cell text="Title" heading-level="6"></ndd-title-cell>');
		await waitForUpdate(el);
		const title = el.shadowRoot!.querySelector('.title-cell__title');
	...
	it('falls back to <p> when heading-level="0"', async () => {
		el = await fixture('<ndd-title-cell text="Title" heading-level="0"></ndd-title-cell>');
		await waitForUpdate(el);
		const title = el.shadowRoot!.querySelector('.title-cell__title');
		expect(title!.tagName.toLowerCase()).toBe('p');
	});
	...
	it('falls back to <p> when heading-level="7"', async () => {
		el = await fixture('<ndd-title-cell text="Title" heading-level="7"></ndd-title-cell>');
		await waitForUpdate(el);
		const title = el.shadowRoot!.querySelector('.title-cell__title');
		expect(title!.tagName.toLowerCase()).toBe('p');
	});
