...
import { styles, menuBarItemStyles } from './rr-menu-bar.styles.js';
import { template, menuBarItemTemplate } from './rr-menu-bar.template.js';
...
export class RRMenuBar extends LitElement {
	...
	_overflowMenuId = `rr-overflow-${Math.random().toString(36).substring(2, 11)}`;
	...
	private _handleKeyDown = (event: KeyboardEvent): void => {
		const items = Array.from(this.querySelectorAll('rr-menu-bar-item:not([disabled])'));
	...
	private _doHandleOverflow(): void {
		...
		const items = slottedElements.filter(el => el.tagName === 'RR-MENU-BAR-ITEM') as HTMLElement[];
		if (items.length === 0) return;
...
if (!customElements.get('rr-menu-bar')) {
