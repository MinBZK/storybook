...
describe('rr-top-title-bar – terugknop', () => {
	...
	it('shows icon back button when is-compact class is set', async () => {
		...
		const iconBtn = el.shadowRoot!.querySelector('.top-title-bar__icon-back-button rr-icon-button');
	...
	it('sets href on back buttons when back-href is set', async () => {
		...
		await waitForUpdate(el);
		const textBtn = el.shadowRoot!.querySelector('.top-title-bar__text-back-button rr-button');
		const iconBtn = el.shadowRoot!.querySelector('.top-title-bar__icon-back-button rr-icon-button');
...
describe('rr-top-title-bar – is-compact', () => {
	let el: RRTopTitleBar;
